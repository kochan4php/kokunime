  ...
  return (
    <>
      ...
      <div className="flex flex-col justify-start lg:col-start-2 lg:col-end-4 py-4 px-2 lg:px-4 lg:py-0">
        ...
        <h1 className="text-2xl lg:text-3xl font-bold my-6 md:my-0 md:mb-7 selection:bg-violet-500 selection:text-violet-900">
          {anime.title}
        </h1>
...
const AnimeSynopsis = ({ anime }: any) => {
  return (
    <>
      <h1 className="text-3xl mb-7 selection:bg-emerald-500 selection:text-emerald-900 font-bold">Sinopsis</h1>
      <div className="text-base selection:bg-green-500 selection:text-green-900 font-medium">
...
const DownloadOption = ({ anime }: any) => {
  return (
    <div className="flex flex-col gap-8 mt-5">
      {anime.download.map((item: AnimeDownloadOptionType, index: number) => (
        <div key={index}>
          <p className="bg-sky-600 rounded p-2 my-3 text-center text-base selection:bg-violet-700 selection:text-violet-100 font-bold">
            {item.title}
          </p>
...
const LinkDownload = ({ anime }: any) => {
  return (
    <>
      <h1 className="text-3xl mb-3 selection:bg-emerald-500 selection:text-emerald-900 text-center font-semibold hover:text-warning transition-all duration-200">
        Link Download
      </h1>
      <div className="lg:container lg:max-w-4xl font-medium">
        <DownloadOption anime={anime} />
      </div>
    </>
  );
};

const SectionDetailAnime = ({ anime }: any) => {
  return (
    <section>
...
const SectionNewSeriesAnime = ({ slug }: any) => {
  return (
  ...
  return (
    <MainLayout>
      <div className="min-w-full text-white py-3 md:px-4">
        <SectionDetailAnime anime={anime} />
        <SectionNewSeriesAnime slug={slug} />
      </div>
    </MainLayout>
  );
};
