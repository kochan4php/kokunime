...
function getDownloadLinks($: CheerioAPI, wrapperClass: string, urlClass: string, titleClass: string): DownloadType[] {
...
function formatAnimeData($: CheerioAPI): AnimeType[] {
...
export async function getAnimePerPage(page: number): Promise<GetAnimePerPageType> {
  try {
...
export async function getRecommendations(): Promise<RecommendationType[]> {
  try {
    const response = await kusonime.get("/");
    const $ = load(response.data);
    const recommendedAnime: RecommendationType[] = [];
    ...
    logger.log("Error", err.message, err.stack);
    return [] as RecommendationType[];
  }
}

export async function getGenres(): Promise<GenreType[] | null> {
  try {
...
export async function getSeasons(): Promise<SeasonType[] | null> {
  try {
